...
class ApiService {
  ...
  private handlePaginatedResponse<T>(response: AxiosResponse<PaginatedResponse<T>>): PaginatedResponse<T> {
    return response.data;
  }
}
...
export const categoriaApi = {
  getCategorias: async (params?: { tipo?: 'receita' | 'despesa'; ativa?: boolean; page?: number; limit?: number }): Promise<PaginatedResponse<Categoria>> => {
  ...
  getEstatisticasCategorias: async (): Promise<ApiResponse<any>> => {
...
export const bancoApi = {
  ...
  getExtratoBanco: async (id: string, params?: { dataInicio?: string; dataFim?: string; page?: number; limit?: number }): Promise<PaginatedResponse<any>> => {
...
export const cartaoApi = {
  ...
  getProximosVencimentos: async (): Promise<ApiResponse<{ proximosVencimentos: any[] }>> => {
    const response = await apiService['api'].get('/cartoes/proximos-vencimentos');
    return apiService['handleResponse'](response);
  },

  getFaturaCartao: async (id: string, mes: number, ano: number): Promise<ApiResponse<any>> => {
...
export const despesaApi = {
  ...
  getProximasParcelasVencimento: async (): Promise<ApiResponse<{ proximasParcelas: any[] }>> => {
...
export const dashboardApi = {
  getResumoGeral: async (params?: { mes?: number; ano?: number }): Promise<ApiResponse<any>> => {
  ...
  getGraficoDespesasPorCategoria: async (params?: { mes?: number; ano?: number }): Promise<ApiResponse<any>> => {
